...
.bottomnav {
  display: flex; border-top: 1.5px solid var(--border); flex-shrink: 0;
  padding-bottom: env(safe-area-inset-bottom, 0); background: var(--white);
}
.tab-btn {
...
.tab-btn.active::after {
  ...
  width: 28px; height: 2px; background: var(--accent); border-radius: 0 0 3px 3px;
}
...
.total-card {
  background: var(--accent); border-radius: var(--r); padding: 22px 20px; margin-bottom: 18px;
...
.inp, .sel {
  width: 100%; padding: 12px 15px; background: var(--white); border: 1.5px solid var(--border);
  border-radius: 10px; font-family: var(--font); font-size: 15px; font-weight: 700;
  color: var(--text); outline: none; appearance: none; -webkit-appearance: none; transition: border-color 0.15s;
}
...
.preview-box {
  ...
  font-size: 13px; font-weight: 700; display: flex; justify-content: space-between; align-items: center; gap: 8px;
}
...
.toast {
  ...
  background: #111827; color: #fff; padding: 10px 22px; border-radius: 40px;
  font-size: 13px; font-weight: 700; z-index: 999; white-space: nowrap; pointer-events: none;
  animation: tin 0.25s ease, tout 0.3s ease 2.4s forwards;
}
...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [cat, stk, inc] = await Promise.all([
  ...
  const doLogPurchase = async () => {
    const doz = parseFloat(incDoz);
    const price = parseFloat(incPrice);
    ...
    try {
      ...
      if (exists) {
        await db(`incoming?id=eq.${exists.id}`, {
          method: "PATCH",
          prefer: "return=representation",
          body: JSON.stringify({ dozens: exists.dozens + doz, price_per_doz: price }),
        });
      } else {
        await db("incoming", {
          method: "POST",
  ...
  const doArrive = async (inc) => {
    ...
    setSaving(true);
    try {
      // update or delete incoming row
      const remaining = inc.dozens - doz;
      if (remaining <= 0) {
        await db(`incoming?id=eq.${inc.id}`, { method: "DELETE", prefer: "" });
      } else {
        await db(`incoming?id=eq.${inc.id}`, {
          method: "PATCH",
          body: JSON.stringify({ dozens: remaining }),
        });
      }
      // upsert stock
      const existingStock = stock.find(s => s.catalogue_id === inc.catalogue_id);
      if (existingStock) {
        await db(`stock?catalogue_id=eq.${inc.catalogue_id}`, {
          method: "PATCH",
          body: JSON.stringify({ dozens: existingStock.dozens + doz }),
        });
      } else {
        await db("stock", {
          method: "POST",
          body: JSON.stringify({ catalogue_id: inc.catalogue_id, dozens: doz }),
        });
      }
      await loadAll();
      pop(`✔ ${doz} doz arrived — added to warehouse`);
      setArrivingId(null); setArriveDoz("");
    } catch (e) { popErr("Failed to update"); }
    finally { setSaving(false); }
  };
  ...
  const doRemove = async () => {
    ...
    try {
      ...
      if (remaining <= 0) {
        await db(`stock?catalogue_id=eq.${remItem.catalogue_id}`, { method: "DELETE", prefer: "" });
      } else {
        await db(`stock?catalogue_id=eq.${remItem.catalogue_id}`, {
          method: "PATCH",
          body: JSON.stringify({ dozens: remaining }),
        });
      }
      await loadAll();
      pop(`✔ Removed ${doz} doz · ${remItem.cat.name}`);
      setRemId(""); setRemDoz("");
    } catch (e) { popErr("Failed to update"); }
    finally { setSaving(false); }
  };
  ...
  const doAddCatalogue = async () => {
    ...
    try {
      await db("catalogue", {
        method: "POST",
        body: JSON.stringify({ name: newName.trim(), selling_price: price }),
      });
      await loadAll();
    ...
    <>
      ...
      <div className="shell">
        ...
        <div className="topbar">
          ...
          <div className={`sync-dot ${syncState}`} title={syncState === "ok" ? "Connected" : syncState === "loading" ? "Syncing..." : "Connection error"} />
        </div>
          ...
          <div className="page">
            ...
            {tab === 1 && (
              <div>
                ...
                <div style={{ background: "var(--warn)", borderRadius: "var(--r)", padding: "18px 20px", marginBottom: 20, position: "relative", overflow: "hidden" }}>
                  ...
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.55)", marginTop: 6, fontWeight: 600 }}>
                    {incomingWithInfo.length} shipment{incomingWithInfo.length !== 1 ? "s" : ""} pending
                  </div>
                </div>
                ...
                <div className="new-purchase">
                  ...
                  {catalogue.length === 0
                    ...
                    : <>
                        ...
                        <button className="cta cta-blue" disabled={saving} onClick={doLogPurchase}>
                          {saving ? "Saving…" : "Log as Incoming"}
                        </button>
                      </>
                  }
                </div>
            ...
            {tab === 2 && (
              <div className="form-card">
                ...
                <div className="form-sub">Goods sold or taken out of stock.</div>
                {stockWithInfo.length === 0
                  ...
                  : <>
                      <div className="field">
                        ...
                        <div className="sel-wrap">
                          <select className="sel" value={remId} onChange={e => { setRemId(e.target.value); setRemDoz(""); }}>
                            <option value="">— select product —</option>
                            {stockWithInfo.map(s => (
                              <option key={s.catalogue_id} value={s.catalogue_id}>
                                {s.cat.name} ({s.dozens} doz)
                              </option>
                            ))}
                          </select>
                        </div>
                      ...
                      <button className="cta cta-orange" disabled={saving} onClick={doRemove}>
            ...
            {tab === 3 && (
              <div>
                          ...
                          {editingId === c.id && (
                            <div className="inline-edit">
                              ...
                              <div className="inline-edit-row">
                                ...
                                <button className="save-btn" disabled={saving} onClick={() => saveEdit(c.id)}>
                                  {saving ? "…" : "Save"}
                                </button>
                              </div>
                            </div>
                ...
                <div className="add-cat-card">
                  ...
                  <button className="cta cta-blue" disabled={saving} onClick={doAddCatalogue}>
